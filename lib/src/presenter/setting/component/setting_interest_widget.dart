...
class SettingInterestWidget extends ConsumerViewModelWidget<SettingViewModel> {
  const SettingInterestWidget({Key? key}) : super(key: key);

  @override
  Widget buildWidget(
      BuildContext context, WidgetRef ref, SettingViewModel viewModel) {
    final async = ref.watch(viewModel.mapInterestModel);
  ...
  Widget buildChooseCategory(
      MapEntry<InterestModel, bool> entry, SettingViewModel viewModel) {
    return Container(
      height: 30,
